...
| # | Bereich | Fehler / Abweichung | Datei | Empfohlene Anpassung |
|---|---------|---------------------|-------|---------------------|
| 1 | sushi-config.yaml | `id: ch.fhir.ig.ch-ig` und `canonical: http://fhir.ch/ig/ch-ig` sind noch vom Template (ch-ig), nicht EMR-spezifisch | `sushi-config.yaml:1-2` | Anpassen auf `ch.fhir.ig.ch-emr` und `http://fhir.ch/ig/ch-emr` |
| 2 | sushi-config.yaml | `description` ist ein Platzhalter: "Implementation Guide CH IG (R4) for ..." | `sushi-config.yaml:5` | Korrekte Beschreibung einfuegen, z.B. "FHIR Implementation Guide for the Swiss Electronic Emergency Record (eNotfallpass)" |
| 3 | sushi-config.yaml | `version: 1.2.3` mit Kommentar "1.2.2 published in May 2025" - passt nicht zu einem neuen IG | `sushi-config.yaml:11` | Version auf `0.1.0` o.ae. setzen (STU Ballot) |
...
| # | Bereich | Fehler / Abweichung | Datei | Empfohlene Anpassung |
|---|---------|---------------------|-------|---------------------|
| 4 | index.md | Tippfehler: "health **lieracy**" statt "health **literacy**" | `index.md:2` | Korrigieren zu "literacy" |
| 5 | index.md | Tippfehler: "**clinicel** data" statt "**clinical** data" | `index.md:3` | Korrigieren zu "clinical" |
| 6 | index.md | Tippfehler: "**diognose**" statt "**diagnoses**" | `index.md:3` | Korrigieren zu "diagnoses" |
| 7 | index.md Profiltabelle | ChEmrRelatedPerson Parent als "CH IPS Patient" angegeben | `index.md:51` | Korrigieren zu "CH Core RelatedPerson" (FSH: `$ChCoreRelatedPerson`) |
| 8 | index.md Profiltabelle | ChEmrPractitioner Parent als "CH Core Practitioner" angegeben | `index.md:52` | Korrigieren zu "CH IPS Practitioner" (FSH: `$CHIPSPractitioner`) |
| 9 | index.md Profiltabelle | ChEmrDevice Parent als "CH IPS Device" angegeben | `index.md:60` | Korrigieren zu "Device" (Basis FHIR R4, FSH: `Parent: Device`) |
...
| # | Bereich | Fehlend im IG | Konzept-Anforderung | Empfohlene Anpassung |
|---|---------|---------------|---------------------|---------------------|
...
| # | Bereich | Problem | Datei | Empfohlene Anpassung |
|---|---------|---------|-------|---------------------|
